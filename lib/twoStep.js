...
/**
 * Exposing low-level unit of the twostep to provide more flexibility
 */
TwoStep.Group = Group;
...
TwoStep.Steppy = function(/*step1, ..., stepN*/) {
	var steps = slice.call(arguments);
	var callback = steps.pop();
	makeSteppies.apply(null, steps)(callback);
};

function makeSteppies() {
	var steps = slice.call(arguments).map(function(step) {
		return TwoStep.throwIfError(step);
	});
	return TwoStep.fn.apply(null, steps);
};

TwoStep.Steppy.fn = makeSteppies;
